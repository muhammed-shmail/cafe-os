...
/** POST /api/admin/tenants/[id]/activate — re-activate a suspended tenant. */
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await setTenantStatus(params.id, 'active');
  await platformAudit({ adminId: s.adminId, action: 'tenant.activate', targetTenantId: params.id, ip: req.headers.get('x-forwarded-for') });
